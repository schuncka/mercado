...
If strCOD_EVENTO <> "" Then
 ...
 FechaRecordSet objRS 
	...
	For i = 0 To strNUM_DIA_FEIRA - 1
		...
		strSQL = strSQL & ", COUNT(DISTINCT if(c.dt_insert between '"&PrepDataIve(strDIA,False,False)&" 00:00' and '"&PrepDataIve(strDIA,False,False)&" 23:59' and (select count(distinct CODBARRA) from tbl_controle_in where cod_Evento = "&strCOD_EVENTO&" and CODBARRA = c.CODBARRA and dt_insert < '"&PrepDataIve(strDIA,False,False)&"') <= 0,c.CODBARRA,null)) as '"&strLABEL_DIA&" - NOVOS'"
		strSQL = strSQL & ", COUNT(DISTINCT if(c.dt_insert between '"&PrepDataIve(strDIA,False,False)&" 00:00' and '"&PrepDataIve(strDIA,False,False)&" 23:59' and (select count(distinct CODBARRA) from tbl_controle_in where cod_Evento = "&strCOD_EVENTO&" and CODBARRA = c.CODBARRA and dt_insert < '"&PrepDataIve(strDIA,False,False)&"') > 0,c.CODBARRA,null)) as '"&strLABEL_DIA&" - RETORNO'"
		strSQL = strSQL & ", COUNT(DISTINCT if(c.dt_insert between '"&PrepDataIve(strDIA,False,False)&" 00:00' and '"&PrepDataIve(strDIA,False,False)&" 23:59',c.CODBARRA,null)) as '"&strLABEL_DIA&" - TOTAL'"
	
	Next
	
	strSQL = strSQL & ", count(distinct c.CODBARRA) as 'TOTAL UNICAS PERIODO'"
 ...
 strFILENAME = "PGEV_RESUMO_" & strCOD_EVENTO & "_" & strFILENAME & "_" & Replace(Date,"/","") & "_" & Replace(Time,":","")
