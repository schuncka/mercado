...
If Request("var_combo") <> "" Then
	...
	sConnectionString ="driver=Provider=MSDASQL;driver={MySQL ODBC 5.1 Driver};server="&CFG_PATH&";uid="&CFG_DB_DADOS_USER &";pwd="&CFG_DB_DADOS_PWD&";database="&CFG_DB_DADOS
'	sConnectionString = "Driver={Microsoft Access Driver (*.mdb)};DBQ=" & Request("var_combo") & ";"
Else
	sConnectionString ="driver=Provider=MSDASQL;driver={MySQL ODBC 5.1 Driver};server="&CFG_PATH&";uid="&CFG_DB_DADOS_USER &";pwd="&CFG_DB_DADOS_PWD&";database="&CFG_DB_DADOS
...
Selecionar conexão:<br><br>
	<select name="var_combo" onChange="javascript:document.frmConexao.submit();">
		<option value="<%=CFG_PATH & CFG_DB_DADOS%>" <% If Request("var_combo") = CFG_DB_DADOS Then Response.Write(" selected")%>>DADOS - <%=CFG_DB_DADOS%></option>
		<option value="<%=CFG_PATH & CFG_DB_DADOS%>" <% If Request("var_combo") = CFG_DB_DADOS Then Response.Write(" selected")%>>CSM - <%=CFG_DB_DADOS%></option>
	</select>
<br><br>
</form>
<%
