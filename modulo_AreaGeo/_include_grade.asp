                    ...
                    <div class="button-dropdown place-left" style="width:20px; height:20px; border:0px solid #F00;">
                           <img class="dropdown-toggle" src="../img/icon_action.gif"  >                            
                             ...
                             <!--DELETE comentado pois neste modo procede que não haja exclusões de registros sem a devida supervisão//-->
                                <li><%=AthWindow("update.asp?var_chavereg="& Cint(GetValue(objRS,DKN)),520,620,"ALTERAR")%></li>
                                <li>
                                <a class="dropdown-toggle" href="#">EXECUTAR</a>
                                   
                                    <% ' DIREITO DE EXECUTAR RELATORIOS
                                  '  if  VerificaDireito ("|RUN|", BuscaDireitosFromDB("modulo_AreaGeo",Session("METRO_USER_ID_USER")), false) = true then
                                    %>
                                     <ul class="dropdown-menu " data-show="hover" data-role="dropdown">
                                        <li><a href="javascript:AbreJanelaPAGE('ReportPrintExport.asp?var_chavereg=<%=Cint(GetValue(objRS,DKN))%>&var_acao=.xls','480', '360')">EXPORTAR EXCEL</a></li>
                                        <li><a href="javascript:AbreJanelaPAGE('ReportPrintExport.asp?var_chavereg=<%=Cint(GetValue(objRS,DKN))%>&var_acao=.doc','480', '360')">EXPORTAR WORD</a></li>
                                        <li><a href="javascript:AbreJanelaPAGE('ReportPrintExport.asp?var_chavereg=<%=Cint(GetValue(objRS,DKN))%>&var_acao=printall','480', '360')">IMPR.TUDO</a></li>      
                                        <!--<li><a href="javascript:AbreJanelaPAGE('ReportPrintExport.asp?var_chavereg=<'%=GetValue(objRS,DKN)%>&var_acao=.xls','480', '360')">EXPORTAR EXCEL</a></li>//-->
                                     </ul>
									<% 
                                   ' end if
                                    %>
                                ...
                                <li><%=AthWindow("detail.asp?var_chavereg="& Cint(GetValue(objRS,DKN)),520,620,"VISUALIZAR")%></li>
                                <li class="divider">
									<%
                                  '  if  VerificaDireito ("|DEL|", BuscaDireitosFromDB("modulo_AreaGeo",Session("METRO_USER_ID_USER")), false) = true then
                                    %>
...
<form name="formReportPrintExport" action="ReportPrintExport.asp" method="post" target="_blank">
  ...
  <input name="var_Id_Areageo" type="hidden" value="<%=strCODAREA%>">
  <input name="var_area" type="hidden" value="<%=strAREA%>">
</form>
