...
<table width="100%" border="0" bgcolor="#FFFFFF">
  <tr>
	...
	<table width="100%"  class="bordasimples" >
        <tr>
          <td width="9%" align="50%" style="border-right:none">&nbsp;&nbsp;<img width="76" height="65" src="../img/logo_ministerio_da_fazenda.gif"><br>          </td>
          <td width="42%" style="border-left:none" ><font size="4"> Ministerio da Fazenda<br>
            Secretaria da Receita Federal</font> </td>
          <td width="49%" align="50%" ><div align="center"><font size="2">COMPROVANTE ANUAL DE RENDIMENTO PAGOS OU
CREDITADOS E DE RETENÇÃO DE IMPOSTO DE
RENDA NA FONTE - PESSOA JURÍDICA</font><br>
              <br>
	...
	Sum(ped_nota_fiscal.valornf) AS valornf
	, date_part('MONTH', ped_nota_fiscal.datanf) AS mesnum
    
	,CASE WHEN date_part('MONTH', ped_nota_fiscal.datanf) = '01' THEN
          'JAN'
       ELSE CASE WHEN date_part('MONTH', ped_nota_fiscal.datanf) = '02' THEN
              'FEV'
          ELSE CASE WHEN date_part('MONTH', ped_nota_fiscal.datanf) = '03' THEN
                  'MAR'
              ELSE CASE WHEN date_part('MONTH', ped_nota_fiscal.datanf) = '04' THEN
                      'ABR'
                  ELSE CASE WHEN date_part('MONTH', ped_nota_fiscal.datanf) = '05' THEN
                          'MAI'
                      ELSE CASE WHEN date_part('MONTH', ped_nota_fiscal.datanf) = '06' THEN
                              'JUN'
                          ELSE CASE WHEN date_part('MONTH', ped_nota_fiscal.datanf) = '07' THEN
                                  'JUL'
                              ELSE CASE WHEN date_part('MONTH', ped_nota_fiscal.datanf) = '08' THEN
                                      'AGO'
                                  ELSE CASE WHEN date_part('MONTH', ped_nota_fiscal.datanf) = '09' THEN
                                          'SET'
                                      ELSE CASE WHEN date_part('MONTH', ped_nota_fiscal.datanf) = '10' THEN
                                              'OUT'
                                          ELSE CASE WHEN date_part('MONTH', ped_nota_fiscal.datanf) = '11' THEN
                                                  'NOV'
                                              ELSE CASE WHEN date_part('MONTH', ped_nota_fiscal.datanf) = '12' THEN
                                                      'DEZ'
                                              END
                                          END
                                      END
  ...
  , ('1708') as cod_ret 
	...
	, Sum(ped_nota_fiscal.valorir) AS irrf
	, cad_empresa.erodape
	, cad_empresa.erazao
	, ped_nota_fiscal.cgcmfnf
	, cad_empresa.ecnpj
	...
	, date_part('YEAR', ped_nota_fiscal.datanf) AS ano
...
FROM 
	...
	ON cad_empresa.idmercado = ped_nota_fiscal.idmercado
WHERE 
	((ped_nota_fiscal.valorir > 0)
	AND ((ped_nota_fiscal.datanf) 
...
GROUP BY
	date_part('MONTH', ped_nota_fiscal.datanf)
  ...
  END
	...
	, ped_nota_fiscal.codigonf
	, ped_nota_fiscal.razaonf
	, date_part('YEAR', ped_nota_fiscal.datanf)
	, ped_nota_fiscal.endereconf
	, ped_nota_fiscal.bairronf
	, ped_nota_fiscal.cidadenf
	, ped_nota_fiscal.estadonf
	...
	, ped_nota_fiscal.idmercado
HAVING 
	(ped_nota_fiscal.idmercado = '$id_empresa');";
	  ...
	  ?>
        <tr>
          ...
          <td width="25%" align="right"><?php echo number_format(getValue($objRSsecundario,"valornf"), 2, ',', '.'); ?> </td>
          <td width="25%" align="right"><?php echo number_format(getValue($objRSsecundario,"irrf"), 2, ',', '.'); ?> </td>
        </tr>
        <?php } ?>
      </table>
