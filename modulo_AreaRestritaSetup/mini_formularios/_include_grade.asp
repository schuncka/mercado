                    ...
                    <div class="button-dropdown place-left" style="width:20px; height:20px; border:0px solid #F00;">
                           <img class="dropdown-toggle" src="../../img/icon_action.gif"  >                            
                             ...
                             <!--DELETE comentado pois neste modo procede que não haja exclusões de registros sem a devida supervisão//-->
                                <li><%=AthWindowNew ("update.asp?var_chavereg="& getValue(objRS,"COD_FORMULARIO"), 520, 620, "ALTERAR")%></li>                                
                                <li><%=AthWindowNew ("detail.asp?var_chavereg="& getValue(objRS,"COD_FORMULARIO"), 520, 620, "VISUALIZAR")%></li>
                                <li>
                                    <a class="dropdown-toggle" href="#">AGREGAÇÔES</a>
                                    <ul class="dropdown-menu " data-show="hover" data-role="dropdown">
                                        <li><%=AthWindow ("mini_servicos_vinculados/default.asp?var_chavereg="&GetValue(objRS,DKN)&"&VAR_COD_EVENTO="&Session("cod_evento"),720, 620, "SERVICOS VINCULADOS")%></li> 
                                ...
                                if  VerificaDireito ("|DEL|", BuscaDireitosFromDB("mini_formularios",Session("METRO_USER_ID_USER")), false) = true then
                                  %>
                                      <li><a href="#" style="cursor:pointer;" onClick="Javascript:data_CheckAll('msguid_',false); document.getElementById('msguid_<%=i%>').checked=true; data_DeleteSelect('msguid_','','<%=LTB%>','<%=CFG_DB%>','<%=DKN%>','<%=DLD&"?var_cod_evento="&strCOD_EVENTO%>','../'); return false;">DELETAR</a></li>
       ...
       <td colspan="<%=UBound(arrFields)+3%>" style="padding-top:3px; border-top:1px solid #000;  background-color:#F8F8F8;" >
            <div style="width:100%; height:35px;">
                 ...
                 <div style="width:180px; height:25px; float:left; text-align:left; border:0px solid #F00; padding-left:25px;">
                   ...
                   <%
			''	if  VerificaDireito("|DEL|", BuscaDireitosFromDB("mini_ListaFormPgto",Session("METRO_USER_ID_USER")), false) then
				 ' %>
                    <button id="butdelete" name="butdelete" class="button link" alt="Apagar Todos Selecionados" onClick="data_DeleteSelect('msguid_','','<%=LTB%>','<%=CFG_DB%>','<%=DKN%>','<%=DLD&"?var_cod_evento="&strCOD_EVENTO%>','../'); return false;">
			...
			''	end if
                 ...
                 <div style="width:150px; height:28px; float:right; text-align:center; border:1px solid #00ADEF; border-radius: 25px; background-color:#00ADEF; margin-right:5px; margin-top:5px; padding-top:0px;" >
                  <form name="formPaginar" id="formPaginar" action="default.asp<%="?var_chavereg="&strCOD_EVENTO%>" method="post">
