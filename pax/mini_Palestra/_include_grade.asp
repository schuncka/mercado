    ...
    <tbody>
        ...
        <!--Menu Action INI-----------------------------------------------------------------------------------------------------//-->
             <td width="3px" align="center">
                    <!-- Comentamos a UPDATE porque no PAX OLD não havia mais essa opção de edição da confirmação 
          ...
          <% 
            ...
            for j=0 to objRS.Fields.count-1 
              if inStr(strFields, objRS.Fields(j).name)>0 then
                ...
                if (objRS.Fields(j).name = "CONFIRMADO") Then
					if (strINFO <> "") Then
						if (strINFO = "1" ) Then
							strINFO = Response.Write("<i class='icon-checkmark fg-green' data-hint-position='left' data-hint='" & objLang.SearchIndex("mini_palestra_conf",0) & "'></i>")	
						else
							strINFO = Response.Write("<i class='icon-cancel-2 fg-red' data-hint-position='left' data-hint='"& objLang.SearchIndex("mini_palestra_nao_conf",0) & "'></i>")		  
						End If
					Else
	                   strINFO = Response.Write("<i class='icon-help fg-yellow' data-hint-position='left' data-hint='"& objLang.SearchIndex("mini_palestra_indefinido",0) & "'></i>")					
					End if
                End If
