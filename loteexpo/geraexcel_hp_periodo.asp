...
strSQL = "SELECT concat(tbl_empresas.cod_empresa,'010') as CODBARRA, tbl_Empresas.COD_EMPRESA, tbl_Empresas.NOMECLI AS RAZAO, tbl_Empresas.NOMEFAN AS FANTASIA, tbl_Empresas.ID_NUM_DOC1 AS 'CPF_CNPJ', IF(tbl_Empresas.TIPO_PESS = 'N','PJ','PF') AS TIPO, tbl_Empresas.END_FULL AS ENDEREÇO, tbl_Empresas.END_BAIRRO AS BAIRRO, tbl_Empresas.END_CIDADE AS CIDADE, tbl_Empresas.END_ESTADO AS UF, tbl_Empresas.END_CEP AS CEP, tbl_Empresas.END_PAIS AS PAIS, tbl_Empresas.FONE1, tbl_Empresas.FONE2, tbl_Empresas.FONE3, tbl_Empresas.FONE4, tbl_Empresas.EMAIL1 AS EMAIL, tbl_empresas.HOMEPAGE as 'Site', tbl_status_cred.STATUS, tbl_empresas.CODATIV1, if(tbl_atividade_pai.ATIVIDADE is null,tbl_atividade.ATIVIDADE,concat(tbl_atividade_pai.ATIVIDADE,' - ',tbl_atividade.ATIVIDADE)) as 'ATIVIDADE', tbl_Empresas.ENTIDADE, tbl_Empresas.DT_NASC, tbl_Empresas.SYS_INATIVO AS EXCLUIDO"&_
",min(if(date_format(tbl_controle_in.dt_insert,'%Y') = '2010' and tbl_controle_in.COD_EVENTO = 243,'X',NULL)) as HP2010"&_
",min(if(date_format(tbl_controle_in.dt_insert,'%Y') = '2011' and tbl_controle_in.COD_EVENTO = 250,'X',NULL)) as HP2011"&_
",min(if(date_format(tbl_controle_in.dt_insert,'%Y') = '2012' and tbl_controle_in.COD_EVENTO = 273,'X',NULL)) as HP2012"&_
",min(if(date_format(tbl_controle_in.dt_insert,'%Y') = '2013' and tbl_controle_in.COD_EVENTO = 828,'X',NULL)) as HP2013"&_
",min(if(date_format(tbl_controle_in.dt_insert,'%Y') = '2014' and tbl_controle_in.COD_EVENTO = 835,'X',NULL)) as HP2013"&_
" FROM tbl_Empresas  LEFT JOIN tbl_Empresas_Sub ON tbl_empresas.cod_empresa = tbl_empresas_sub.cod_empresa " &_
"                    LEFT JOIN tbl_controle_in ON tbl_controle_in.codbarra = if(tbl_empresas_sub.codbarra is null,concat(tbl_empresas.cod_empresa,'010'),tbl_empresas_sub.codbarra) and tbl_controle_in.COD_EVENTO in (243,250,273,828,835)"&_
...
Else
 strFILENAME = "SAC25297_HP_2010_ATE_2014_" & strCOD_EVENTO & "_" & strFILENAME & "_" & Replace(Date,"/","") & "_" & Replace(Time,":","")
End If
