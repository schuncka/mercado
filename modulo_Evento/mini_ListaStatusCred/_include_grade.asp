                    ...
                    <div class="button-dropdown place-left" style="width:20px; height:20px; border:0px solid #F00;">
                           <img class="dropdown-toggle" src="../../img/icon_action.gif"  >                            
                             ...
                             <!--DELETE comentado pois neste modo procede que não haja exclusões de registros sem a devida supervisão//-->
                                <li><%=AthWindowNew ("update.asp?var_chavereg="& getValue(objRS,DKN), 520, 620, "ALTERAR")%></li>                                
                                <li><%=AthWindowNew ("detail.asp?var_chavereg="& getValue(objRS,DKN), 520, 620, "VISUALIZAR")%></li>
                                <li class="divider">
                                  <%
								    							
								 '  if  VerificaDireito ("|DEL|", BuscaDireitosFromDB("mini_BreteCred",Session("METRO_USER_ID_USER")), false) = true then
								   %>
                                <li><a href="#" style="cursor:pointer;" onClick="Javascript:data_CheckAll('msguid_',false); document.getElementById('msguid_<%=i%>').checked=true; data_DeleteSelect('msguid_','','<%=LTB%>','<%=CFG_DB%>','<%=DKN%>','<%=DLD& "?var_chavereg="&strCOD_STATUS_CRED%>','../'); return false;">DELETAR</a></li>
									<%
								'	end if
									%>
                            </ul>
            ...
            for j=0 to objRS.Fields.count-1 
              if inStr(strFields, objRS.Fields(j).name)>0 then
                ...
                strINFO = GetValue(objRS,objRS.Fields(j).name)						
					If (objRS.Fields(j).name = "BACKGROUND_SHOW") Then 
						if (strINFO="1") Then
					...
					If (objRS.Fields(j).name = "CAEX_LOGIN") Then 
						if (strINFO="1") Then
							 strINFO = Response.Write("<i <i class='icon-checkmark fg-green'></i>")
						Else 
							strINFO = Response.Write("<i class='icon-cancel-2'></i>")
					...
					If (objRS.Fields(j).name = "CAEX_SHOW") Then 
						if (strINFO="1") Then
							 strINFO = Response.Write("<i <i class='icon-checkmark fg-green'></i>")
						Else 
							strINFO = Response.Write("<i class='icon-cancel-2'></i>")
					...
					If (objRS.Fields(j).name = "CONTATO_SHOW") Then 
						if (strINFO="1") Then
							 strINFO = Response.Write("<i <i class='icon-checkmark fg-green'></i>")
						Else 
							strINFO = Response.Write("<i class='icon-cancel-2'></i>")
					...
					If (objRS.Fields(j).name = "BGCOLOR") Then 
						if (strINFO <> "") Then
						'strINFO =response.Write(getValue(objRS,"BGCOLOR"))
						strINFO = Response.Write("<div class='#' style='background:"&getValue(objRS,"BGCOLOR")&"'>"&getValue(objRS,"BGCOLOR")&"</div>")
						END IF
					end if	
       ...
       <td colspan="<%=UBound(arrFields)+3%>" style="padding-top:3px; border-top:1px solid #000;  background-color:#F8F8F8;" >
            <div style="width:100%; height:35px;">
                 ...
                 <div style="width:180px; height:25px; float:left; text-align:left; border:0px solid #F00; padding-left:25px;">
				   ...
				   'if  VerificaDireito ("|DEL|", BuscaDireitosFromDB("mini_BreteCred",Session("METRO_USER_ID_USER")), false) = true then
				   %>
                    <button id="butdelete" name="butdelete" class="button link" alt="Apagar Todos Selecionados" onClick="data_DeleteSelect('msguid_','','<%=LTB%>','<%=CFG_DB%>','<%=DKN%>','<%=DLD& "?var_chavereg="&strCOD_STATUS_CRED%>%>','../'); return false;">
                 ...
                 <div style="width:150px; height:28px; float:right; text-align:center; border:1px solid #00ADEF; border-radius: 25px; background-color:#00ADEF; margin-right:5px; margin-top:5px; padding-top:0px;" >
                  <form name="formPaginar" id="formPaginar" action="default.asp<%="?var_cod_evento="&strCOD_EVENTO%>" method="post">
