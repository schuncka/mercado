                    ...
                    <div class="button-dropdown place-left" style="width:20px; height:20px; border:0px solid #F00;">
                           <img class="dropdown-toggle" src="../img/icon_action.gif"  >                            
                             ...
                             <!--DELETE comentado pois neste modo procede que não haja exclusões de registros sem a devida supervisão//-->
                                ...
                                <li><%=AthWindow ("update.asp?var_chavereg=" & GetValue(objRS,DKN)&"&var_cod_evento="&GetValue(objRS,"COD_EVENTO"),620, 775, "ALTERAR")%></li>
                                <li>
                                    <a class="dropdown-toggle" href="#">AGREGAÇÔES</a>
                                    <ul class="dropdown-menu " data-show="hover" data-role="dropdown">
                                        <!--<li><'%=AthWindow ("mini_DadosRpsNfe/default.asp?var_chavereg="&GetValue(objRS,DKN)&"&var_cod_evento="& GetValue(objRS,"COD_EVENTO"), 720, 620, "DADOS NPS/NFE")%></li>-->
                                        <li><%=AthWindow ("mini_ListaFormaPgto/default.asp?var_chavereg="&GetValue(objRS,"ID_AUTO")&"&var_cod_evento="&GetValue(objRS,"COD_EVENTO")   ,720, 620, "FORMA PGTO")%></li>
                                        <li><%=AthWindow ("mini_ListaImagens/default.asp?var_chavereg="&GetValue(objRS,"ID_AUTO")&"&var_cod_evento="&GetValue(objRS,"COD_EVENTO")   ,720, 620, "IMAGENS")%></li>
                                        <li><%=AthWindow ("mini_ListaStatusCred/default.asp?var_chavereg="&GetValue(objRS,"ID_AUTO")&"&var_cod_evento="&GetValue(objRS,"COD_EVENTO")&"&var_status_cred="&GetValue(objRS,"STATUS_CRED")   ,820, 620, "TIPO CRED")%></li>
                                        <li><%=AthWindow ("mini_ListaCategoria/default.asp?var_chavereg="&GetValue(objRS,"ID_AUTO")&"&var_cod_evento="&GetValue(objRS,"COD_EVENTO") ,720, 620, "CATEGORIA")%></li>
                                        <li><%=AthWindow ("mini_DadosRpsNfe/default.asp?var_chavereg="&GetValue(objRS,"ID_AUTO")&"&var_cod_evento="&GetValue(objRS,"COD_EVENTO")   ,720, 620, "DADOS RPS/NFE")%></li>
                                        <li class="divider">
                                        <li><%=AthWindow ("Cfg_PvistaApp.asp?var_chavereg=" & GetValue(objRS,DKN)&"&var_cod_evento="&GetValue(objRS,"COD_EVENTO"),630, 725, "CFG APP")%></li>
                                        <li><%=AthWindow ("mini_CfgLinks/default.asp?var_chavereg="&GetValue(objRS,"ID_AUTO")&"&var_cod_evento="&GetValue(objRS,"COD_EVENTO")   ,720, 620, "CFG LINKS")%></li>
                                    </ul>
                               </li>
                                <li class="divider">
                                <li><%=AthWindow ("detail.asp?var_chavereg=" & GetValue(objRS,DKN),720, 620, "VISUALIZAR")%></li>
                                <li><a href="#" style="cursor:pointer;" onClick="Javascript:data_CheckAll('msguid_',false); document.getElementById('msguid_<%=i%>').checked=true; data_DeleteSelect('msguid_','deleteexec.asp','<%=LTB%>','<%=CFG_DB%>','<%=DKN&"&var_cod_evento="&GetValue(objRS,"COD_EVENTO")%>','<%=DLD%>',''); return false;">DELETAR</a></li>
                            </ul>
                    </div>                        
             </td>
        <!--Menu Action FIM-----------------------------------------------------------------------------------------------------//-->
          ...
          <% 
            for j=0 to objRS.Fields.count-1 
              if inStr(strFields, objRS.Fields(j).name)>0 then
                ...
                'Tratamento especial exibição campo LOJA-ATIVA
                If (objRS.Fields(j).name = "LOJA_ATIVA") Then
                    IF (strINFO="") Then
                        strINFO = "Sim" 
                    Else
                        If Cdate(GetValue(objRS,objRS.Fields(j).name)) < Cdate(Date()) Then
                              strINFO = "<font color='#FF7F00'>" & PrepData(objRS("LOJA_ATIVA"),True,False) & "</font>" 
                        Else		  
                              strINFO = PrepData(objRS("LOJA_ATIVA"),True,False) 
                        End If
                    End If
                End If
                'Tratamento especial exibição campo PSC_ATIVA
                If (objRS.Fields(j).name = "PSC_ATIVA") Then
                    IF (strINFO="") Then
                        strINFO = "Sim" 
                    Else
                        If Cdate(GetValue(objRS,objRS.Fields(j).name)) < Cdate(Date()) Then
                              strINFO = "<font color='#FF7F00'>" & PrepData(objRS("PSC_ATIVA"),True,False) & "</font>" 
                        Else		  
                              strINFO = PrepData(objRS("PSC_ATIVA"),True,False) 
                        End If
                    End If
                End If
                'Tratamento especial para CUPOM FISCAL
                if (objRS.Fields(j).name = "CUPOM_FISCAL") Then
                    if (strINFO="0") Then
       ...
       <td colspan="<%=UBound(arrFields)+3%>" style="padding-top:3px; border-top:1px solid #000;  background-color:#F8F8F8;" >
            <div style="width:100%; height:35px;">
                 ...
                 <div style="width:180px; height:25px; float:left; text-align:left; border:0px solid #F00; padding-left:25px;">
                    <button id="butdelete" name="butdelete" class="button link" alt="Apagar Todos Selecionados" onClick="data_DeleteSelect('msguid_','deleteexec.asp','<%=LTB%>','<%=CFG_DB%>','<%=DKN%>','<%=DLD%>',''); return false">
