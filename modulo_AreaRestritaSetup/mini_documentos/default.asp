...
VerificaDireito "|VIEW|", BuscaDireitosFromDB("mini_documentos",Session("METRO_USER_ID_USER")), true 
 ...
 Dim  strID_AUTO, strCOD_EVENTO, strLANG
...
strSQL = " SELECT id_documento, rotulo, area" &_
	         "  FROM tbl_documentos " &_
			 "  WHERE COD_EVENTO = " & strCOD_EVENTO &_
              "  AND (lang = " & ValidateValueSQL(strLANG,"STR",false) & " or lang is null)"&_
			 " ORDER BY rotulo"
			 		  
 'athDebug strSQL , FALSE

 ' Define os campos para exibir na grade
 strFields = "id_documento,  rotulo, area "  
 arrFields = Split(strFields,",")        
 arrFields = Split(strFields,",")        

 arrLabels = Array("ID"               ,"ROTULO"     , "AREA"     )
