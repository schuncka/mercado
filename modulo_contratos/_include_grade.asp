                    ...
                    <div class="button-dropdown place-left" style="width:20px; height:20px; border:0px solid #F00;">
                           <img class="dropdown-toggle" src="../img/icon_action.gif"  >                            
                             ...
                             <!--DELETE comentado pois neste modo procede que não haja exclusões de registros sem a devida supervisão//-->
                                ...
                                <li>
                                    <a class="dropdown-toggle" href="#">IMP. CONTRATOS</a>
                                    <ul class="dropdown-menu " data-show="hover" data-role="dropdown">
                                		  <li><%=AthWindow ("contrato.asp?var_chavereg=" & GetValue(objRS,DKN) & "&var_tipo=m_comprador", 620, 620, "M.Comprador")%></li>
                                      <li><%=AthWindow ("contrato.asp?var_chavereg=" & GetValue(objRS,DKN) & "&var_tipo=m_vendedor",  620, 620, "M.Vendedor")%></li>
                                      <li><%=AthWindow ("contrato.asp?var_chavereg=" & GetValue(objRS,DKN) & "&var_tipo=s_comprador", 620, 620, "S.Comprador")%></li>
                                      <li><%=AthWindow ("contrato.asp?var_chavereg=" & GetValue(objRS,DKN) & "&var_tipo=s_vendedor",  620, 620, "S.Vendedor")%></li>
                                    <!--<li><%=AthWindow ("mini_ListaPrecosAreaRestrita/?var_chavereg=" & GetValue(objRS,DKN)&"&var_cod_evento="&getValue(objRS,"cod_evento"), 520, 620, "LISTA PREÇO AR")%></li>
                                     <li><%=AthWindow ("mini_ListaRestricao/?var_chavereg=" & GetValue(objRS,DKN)&"&var_cod_evento="&getValue(objRS,"cod_evento"), 520, 620, "LISTA RESTRIÇÃO PROD")%></li> -->
                                     <!--<li><%=AthWindow ("mini_Contatos/default.asp?var_chavereg=" & GetValue(objRS,"codigodocliente"), 520, 620, "CONTATOS")%></li>
                                     <li><%=AthWindow ("mini_Produtos/default.asp?var_chavereg=" & GetValue(objRS,"codigodocliente"), 520, 620, "PRODUTOS")%></li>
                                     <li><%=AthWindow ("mini_Marcas/default.asp?var_chavereg=" & GetValue(objRS,"COD_TBL_CLIENTES"), 520, 620, "MARCAS")%></li>-->
                                	</ul>
                                </li>
                                <li class="divider">
								<%'DIREITOS DE DELETAR RELATORIOS
                                'if  VerificaDireito ("|DEL|", BuscaDireitosFromDB("modulo_AdmServico",Session("METRO_USER_ID_USER")), false) = true then
                                %>
                                	<li><a href="#" style="cursor:pointer;" onClick="Javascript:data_CheckAll('msguid_',false); document.getElementById('msguid_<%=i%>').checked=true; data_DeleteSelect('msguid_','deleteexec.asp','<%=LTB%>','<%=CFG_DB%>','<%=DKN%>','<%=DLD%>',''); return false;">DELETAR</a></li>
                                <%'DIREITOS DE DELETAR RELATORIOS
                               ' end if               
                                %>
                            </ul>
