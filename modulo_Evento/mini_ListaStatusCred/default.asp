...
<% VerificaDireito "|VIEW|", BuscaDireitosFromDB("mini_ListaStatusCred",Session("METRO_USER_ID_USER")), true %>
<%
...
' ---------------------------------------------------------------------------------------------------------------
 ...
 strSQL = " SELECT DISTINCT SC.COD_STATUS_CRED "
 ...
 strSQL = strSQL & "		   FROM tbl_STATUS_CRED AS SC, tbl_EVENTO as EV " 
 'strSQL = strSQL & "           where SC.COD_STATUS_CRED = " & strSTATUSCRED
 'strSQL = strSQL & "           WHERE SC.COD_STATUS_CRED = EV.STATUS_CRED "
 strSQL = strSQL & "           ORDER BY SC.COD_STATUS_CRED, SC.ORDEM " 
 ...
 strFields = "SC.COD_STATUS_CRED, SC.STATUS, SC.TIPOPESS, SC.CONTATO_SHOW, SC.CAEX_SHOW, SC.CAEX_LOGIN,SC.ORDEM,SC.BACKGROUND_SHOW,SC.BGCOLOR" 
 arrFields = Split(strFields,",")        

 arrLabels = Array("COD"              ,"TIPO CRED"           ,"T.PESS"           ,  "CONTATOSHOW"     ,  "CAEXSHOW"    ,  "CAEXLOGIN" , "ORDEM"     ,"BGSHOW" ,""         )
 arrSort   = Array("sortable-numeric" ,"sortable-numeric" ,"sortable-numeric" , "sortable-numeric" , "sortable"   , "sortable", "sortable" , "sortable"    ,"sortable" ) 'Obs.:"sortable-date-dmy", "sortable-currency", "sortable-numeric", "sortable" 
 arrWidth  = Array("2%"               ,"10%"              ,     "15%"  	      ,   "15%"            ,    "15%"     ,    "15%" ,    "5%"    ,   "11%"        ,"3%"      )     'Obs.:[somar 98%] ou deixar todos vazios
' ----------------------------------------------------------------------------------------------------------------------------
...
<!-- Embora um MINI Módulo esteja convencionado a não ter elementos de filtragem
     ...
     <form id="formfiltro" name="formfiltro" action="default.asp" method="post" style="display:none; visibility:hidden;" >
      ...
      <input type="hidden" id="var_chavereg"   name="var_chavereg"   value="<%=strCOD_STATUS_CRED%>">
      <input type="hidden" id="var_status_cred"   name="var_status_cred"   value="<%=strSTATUSCRED%>">
...
<!--                <p class="button bg-dark fg-white"><'%=AthWindow("INSERT.ASP", 520, 620, "ADICIONAR")%></p>&nbsp;//-->                
					<p class="button bg-dark fg-white"><a href="#" onClick="javascript:AbreJanelaPAGE_NOVA('insert.asp?var_chavereg=<%=strCOD_STATUS_CRED%>','520','620')">ADICIONAR</a></p>&nbsp;                
