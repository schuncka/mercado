                    ...
                    <div class="button-dropdown place-left" style="width:20px; height:20px; border:0px solid #F00;">
                           <img class="dropdown-toggle" src="../img/icon_action.gif"  >                            
                             ...
                             <!--DELETE comentado pois neste modo procede que não haja exclusões de registros sem a devida supervisão//-->
                                ...
                                <li class="divider">
                                  <%
								   if  VerificaDireito ("|DEL|", BuscaDireitosFromDB("modulo_MapeamentoCampo",Session("METRO_USER_ID_USER")), false) = true then
								   %>
          ...
          <% 
            for j=0 to objRS.Fields.count-1 
              if inStr(strFields, objRS.Fields(j).name)>0 then
				...
				'Tratamento especial CAMPO COR DESTAQUE
                if (objRS.Fields(j).name = "CAMPO_COR_DESTAQUE") Then
	        	 	strINFO = Response.Write("<div style='width:30px; height:20px; background-color:" & strINFO & ";'></div>")	 
                End If
