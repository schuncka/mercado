...
VerificaDireito "|VIEW|", BuscaDireitosFromDB("mini_formularios",Session("METRO_USER_ID_USER")), true 
 ...
 Dim  strID_AUTO, strCOD_EVENTO, strCOD_FORMULARIO, strCOD_DADO
...
strSQL = " SELECT cod_formulario, cod_evento, rotulo," &_
             "  DATE_FORMAT(dt_inativo, '%d/%m/%Y') as dead_line" &_ 
	         "  FROM tbl_formularios " &_
			 "  WHERE cod_evento = " & strCOD_EVENTO &_
             "  ORDER BY 3"
			 		  
 'athDebug strSQL , false

 ' Define os campos para exibir na grade
 strFields = "cod_formulario, cod_evento, rotulo, dead_line"  
 arrFields = Split(strFields,",")        
 arrFields = Split(strFields,",")        

 arrLabels = Array("ID"        ,"CÓD"              ,"ROTULO"   , "DEAD LINE")
