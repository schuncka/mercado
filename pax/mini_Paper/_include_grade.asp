...
<!-- Possibilidades de tipo de sort...  class="sortable-date-dmy" / class="sortable-currency" / class="sortable-numeric" / class="sortable" //-->
    <thead>
        <tr> 
         <th style="width:1%;"></th>
         <th style="width:1%;padding:0px; vertical-align:middle; text-align:center " nowrap>
		 <!-- <span class="icon-checkbox" onClick="Javascript:data_ToggleCheckAll('msguid_'); return false;" style="cursor:pointer;"></span> //-->
		 </th>    
    ...
    <tbody>
                    ...
                    <div class="button-dropdown place-left" style="width:20px; height:20px; border:0px solid #F00;">
                           <img class="dropdown-toggle" src="../../img/icon_action.gif"  >                            
                             <ul class="dropdown-menu" data-role="dropdown">
                                <li><%=AthWindowNew ("certificadopaperpdf.asp?var_cod_paper_cadastro=" & GetValue(objRS,"COD_PAPER_CADASTRO") & "&var_cod_evento="&GetValue(objRS,"COD_EVENTO")& "&var_cod_empresa=" & strCOD_EMPRESA, 520, 620,ucase(objLang.SearchIndex("mini_cert_imprimir",0))  )%></li>                                
                                <li><%=AthWindowNew ("edit_trab.asp?var_chavereg="& GetValue(objRS,"COD_PAPER_CADASTRO") &"&lng=BR", 520, 620, ucase(objLang.SearchIndex("mini_alterar",0)) )%></li>
								<%If GetValue(objRS,"STATUS")&"" = "" Then %>
                                <li class="divider">
                                <li><a href="#" style="cursor:pointer;" onClick="Javascript:data_CheckAll('msguid_',false); document.getElementById('msguid_<%=i%>').checked=true; data_DeleteSelect('msguid_','deleteexec.asp','<%=LTB%>','<%=CFG_DB%>','<%=DKN%>','<%=DLD%>&var_chavereg=<%=strCOD_EMPRESA%>',''); return false;"><%=ucase(objLang.SearchIndex("mini_deletar",0))%></a></li>
								<% End if %>
                            </ul>
                    </div>                        
             </td>
        <!--Menu Action FIM-----------------------------------------------------------------------------------------------------//-->
             <td align='center'>
			 <!-- <input type='checkbox' value='<%=GetValue(objRS,DKN)%>' name='msguid_<%=i%>' id='msguid_<%=i%>'>//--></td>
          <% 
            for j=0 to objRS.Fields.count -1 
              if inStr(strFields, objRS.Fields(j).name)>0 then
                ...
                if (objRS.Fields(j).name = "STATUS") AND GetValue(objRS,"STATUS")&"" = "" Then 
                        strINFO = Response.Write("EM ANÁLISE")	
                End If
				
                'if (objRS.Fields(j).name = "ISTATUS") AND GetValue(objRS,"ISTATUS")&"" <> "" Then 
				if (objRS.Fields(j).name = "COD_PAPER_STATUS")AND ((CStr(GetValue(objRS,"COD_PAPER_STATUS"))<>"20") AND (CStr(GetValue(objRS,"COD_PAPER_STATUS"))<>"13") AND  (CStr(GetValue(objRS,"COD_PAPER_STATUS"))<>"15")) THEN
                        strINFO = Response.Write("<i class='icon-locked-2' data-hint='Este trabalho possui um status que não permite edição ou deleção' data-hint-position='left'> </i> ")	
                End If				
       ...
       <td colspan="<%=UBound(arrFields)+3%>" style="padding-top:3px; border-top:1px solid #000;  background-color:#F8F8F8;" >
            <div style="width:100%; height:35px;">
                 ...
                 <div style="width:180px; height:25px; float:left; text-align:left; border:0px solid #F00; padding-left:25px;">
                                    <!-- este modulo carrega a data_deleteselect pois deleta apenas uma tabela//-->
                   <%
'				   if  VerificaDireito ("|DEL|", BuscaDireitosFromDB("mini_ListaCategoria",Session("METRO_USER_ID_USER")), false) then
				   %>
				   <!--
                    <button id="butdelete" name="butdelete" class="button link" alt="Apagar Todos Selecionados" onClick="data_DeleteSelect('msguid_','deleteexec.asp','<%=LTB%>','<%=CFG_DB%>','<%=DKN%>','<%=DLD%>&var_chavereg=<%=strCOD_EMPRESA%>',''); return false">  
