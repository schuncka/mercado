                    ...
                    <div class="button-dropdown place-left" style="width:20px; height:20px; border:0px solid #F00;">
                           <img class="dropdown-toggle" src="../../img/icon_action.gif"  >                            
                             ...
                             <!--DELETE comentado pois neste modo procede que não haja exclusões de registros sem a devida supervisão//-->
                                <li><%=AthWindowNew ("update.asp?var_chavereg="& getValue(objRS,DKN) & "&var_chavemaster=" & strCODLOCALCREDENCIAL, 520, 620, "ALTERAR")%></li>                                
                                <li><%=AthWindowNew ("detail.asp?var_chavereg="& getValue(objRS,DKN) & "&var_chavemaster=" & strCODLOCALCREDENCIAL, 520, 620, "VISUALIZAR")%></li>
                                <li class="divider">
                                  <%
								    							
								   if  VerificaDireito ("|DEL|", BuscaDireitosFromDB("mini_BreteCred",Session("METRO_USER_ID_USER")), false) = true then
								   %>
                                <li><a href="#" style="cursor:pointer;" onClick="Javascript:data_CheckAll('msguid_',false); document.getElementById('msguid_<%=i%>').checked=true; data_DeleteSelect('msguid_','','<%=LTB%>','<%=CFG_DB%>','<%=DKN%>','<%=DLD& "?var_chavereg="&strCODLOCALCREDENCIAL%>','../'); return false;">DELETAR</a></li>
									<%
									end if
									%>
                            </ul>
       ...
       <td colspan="<%=UBound(arrFields)+3%>" style="padding-top:3px; border-top:1px solid #000;  background-color:#F8F8F8;" >
            <div style="width:100%; height:35px;">
                 ...
                 <div style="width:180px; height:25px; float:left; text-align:left; border:0px solid #F00; padding-left:25px;">
				   ...
				   if  VerificaDireito ("|DEL|", BuscaDireitosFromDB("mini_BreteCred",Session("METRO_USER_ID_USER")), false) = true then
				   %>
                    <button id="butdelete" name="butdelete" class="button link" alt="Apagar Todos Selecionados" onClick="data_DeleteSelect('msguid_','','<%=LTB%>','<%=CFG_DB%>','<%=DKN%>','<%=DLD& "?var_chavereg="&strCODLOCALCREDENCIAL%>',''); return false;">
