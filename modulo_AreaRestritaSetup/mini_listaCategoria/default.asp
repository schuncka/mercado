...
VerificaDireito "|VIEW|", BuscaDireitosFromDB("mini_listaCategoria",Session("METRO_USER_ID_USER")), true 
 ...
 Const DLD = "../modulo_AreaRestritaSetup/mini_listaCategoria/default.asp" 	' "../evento/data.asp" - 'Default Location após Deleção
 ...
 Dim  strID_AUTO, strCOD_EVENTO, strLANG, strBgColor, strOBSERVACAO, strCOD_DADO
...
strCOD_DADO   = Replace(GetParam("var_chavereg"),"'","''")
'------------------------------------------------------------------
...
strSQL = " SELECT COD_STATUS_PRECO, STATUS, OBSERVACAO, SENHA, LOJA_SHOW " &_
			 " FROM tbl_STATUS_PRECO  " &_
			 " WHERE COD_EVENTO = " & ValidateValueSQL(strCOD_EVENTO,"NUM",false) &_ 
			 " AND CAEX_SHOW = 1 "&_
			 " ORDER BY STATUS, COD_STATUS_PRECO " 
			 		  
 'athDebug strSQL , FALSE

 ' Define os campos para exibir na grade
 strFields = "COD_STATUS_PRECO,    STATUS, OBSERVACAO "  
 arrFields = Split(strFields,",")        
 arrFields = Split(strFields,",")        

 arrLabels = Array("COD"               ,"STATUS"     , "OBSERVAÇÃO")
...
<form id="formfiltro" name="formfiltro" action="default.asp" method="post" style="display:none; visibility:hidden;" >
    ...
    <input type="hidden" id="var_chavereg"   name="var_chavereg"   value="<%=strCOD_DADO%>">
