...
<%
 ...
 Const DLD = "../modulo_FinLctoConta/busca_entidade.asp " 		' "../evento/data.asp" - 'Default Location após Deleção
 Const TIT = "BUSCA ENTIDADE "    						' - Nome/Titulo sendo referencia como titulo do módulo no botão de filtro
 ...
 Dim strSEARCH,strINPUT1,strINPUT2,strINPUT_TIPO,strINPUT_NOME,strVEZES,strFORM,strPALAVRA_CHAVE
...
' Monta FILTRAGEM -----------------------------------------------------------------------------------------------
 ...
 ' response.Write(strSEARCH&"<BR>")
    If isNUmeric(strSEARCH) then 
		 If isNUmeric(strSEARCH) then 
			auxSTR = auxSTR & " AND COD_EMPRESA 	=  '" & Clng(strSEARCH) & "'" 
		' ELSE	
			'auxSTR = auxSTR & " OR ID_AUTO		=  '" & Clng(strSEARCH) & "'" 
		 END IF
		'response.Write(isNUmeric(strSEARCH))
		'response.End()
	else
		If strSEARCH   <>   ""  Then 
			auxSTR = auxSTR & " AND NOMECLI  LIKE   '%" & strSEARCH &     "%'"
		end if
	end if	
...
' Monta SQL e abre a consulta ----------------------------------------------------------------------------------
 ...
 strSQL = strSQL & "		  WHERE SYS_INATIVO IS NULL " & MontaWhereAdds
 strSQL = strSQL & "		  ORDER BY COD_EMPRESA"
 ...
 strFields = "ID_AUTO,COD_EMPRESA,NOMECLI"
 arrFields = Split(strFields,",")        

 arrLabels = Array("ID","COD_EMPRESA","NOMECLI")
 arrSort   = Array("sortable-numeric","sortable-numeric","sortable")
 arrWidth  = Array("2%","49%","49%")  'obs.:[somar 98%] ou deixar todos vazios
' ----------------------------------------------------------------------------------------------------------------------------
        ...
        <div style="width:100%;display:inline-block"> 
            ...
            <div class="grid" style="margin:0px;padding:0px;">
                        <div class="row" style="margin:0px;padding:0px;">
                              <form name="formfiltro" id="formfiltro" method="post" action="busca_entidade.asp" >  
                                <div class="span12">
...
<!-- Possibilidades de tipo de sort...  class="sortable-date-dmy" / class="sortable-currency" / class="sortable-numeric" / class="sortable" //-->
    ...
    <tbody>
		...
		%>
        	 onClick="Retorna('<%=GetValue(objRS,"ID_AUTO")%>','<%=GetValue(objRS,"NOMECLI")%>');"
        <%		
       ...
       <td colspan="<%=UBound(arrFields)+3%>" style="padding-top:3px; border-top:1px solid #000;  background-color:#F8F8F8;" >
            <div style="width:100%; height:35px;">
                 ...
                 <div style="width:150px; height:28px; float:right; text-align:center; border:1px solid #00ADEF; border-radius: 25px; background-color:#00ADEF; margin-right:5px; margin-top:5px; padding-top:0px;" >
                  <form name="formPaginar" id="formPaginar" action="busca_entidade.asp" method="post">
