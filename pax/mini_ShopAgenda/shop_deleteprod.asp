...
<% 'VerificaDireito "|DEL|", BuscaDireitosFromDB("modulo_PaxShopAgenda", Request.Cookies("pVISTA")("ID_USUARIO")), true %>
<%
 ...
 strSQL = "          SELECT p.id_auto, p.cod_prod,ip.cod_inscricao, p.`local` as sala "
...
function ok() { 
 <% if (CFG_WINDOW = "NORMAL") then 
  		response.write ("document.formproduto.DEFAULT_LOCATION.value='athWindowClose.asp';") 
	 else
  		response.write ("document.formproduto.DEFAULT_LOCATION.value='athWindowClose.asp';")  
  	 end if
 %> 
...
<div class="bg-darkRed fg-white" style="width:100%; height:50px; font-size:20px; padding:10px 0px 0px 10px;">
   <!-- <%=strCOD_EVENTO & " - " & strNOME_EVENTO%>&nbsp;<sup><span style="font-size:12px">DELETE</span></sup> //-->
   DELETE&nbsp;<sup><span style="font-size:12px"></span></sup>
</div>
<!-- FIM -------------------------------Barra//-->
<div class="container padding20">
		<form name="formproduto" id="formproduto" action="shop_deleteprod_exec.asp" method="post" target="_self">
		...
		<div class="frames" style="background:#FDFDFD;">
				<div class="padding20">
					<div  style="width:100%; margin-right:5px;">
							<h2><%=objLang.SearchIndex("mini_shopagenda_confdel",0)%></h2>
                            <hr>
