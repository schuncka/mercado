                    ...
                    <div class="button-dropdown place-left" style="width:20px; height:20px; border:0px solid #F00;">
                           <img class="dropdown-toggle" src="../../img/icon_action.gif"  >                            
                             ...
                             <!--DELETE comentado pois neste modo procede que não haja exclusões de registros sem a devida supervisão//-->
                                <li><%=AthWindowNew ("update.asp?var_chavereg="& getValue(objRS,DKN)&"&var_cod_cli="&GetValue(objRS,"CodigoDoCliente") , 520, 620, "ALTERAR")%></li>                                                                
                                <li class="divider">
                                  <%
								    							
								 '  if  VerificaDireito ("|DEL|", BuscaDireitosFromDB("mini_BreteCred",Session("METRO_USER_ID_USER")), false) = true then
								   %>
                                <li><a href="#" style="cursor:pointer;" onClick="Javascript:data_CheckAll('msguid_',false); document.getElementById('msguid_<%=i%>').checked=true; data_DeleteSelect('msguid_','','<%=LTB%>','<%=CFG_DB%>','<%=DKN%>','<%=DLD& "?var_chavereg="&strCOD_EVENTO%>','../'); return false;">DELETAR</a></li>
									<%
								'	end if
									%>
                            </ul>
          ...
          <% 
            for j=0 to objRS.Fields.count-1
              ...
              if inStr(strFields, objRS.Fields(j).name)>0 then
                ...
                if objRS.Fields(j).name = "DOCUMENTO" then

                  strInfo = "<a href='http://servidor.clicmercado.com.br/mercado/_documentos_cliente/" & GetValue(objRS,objRS.Fields(j).name) & "' download>" & GetValue(objRS,objRS.Fields(j).name) &"</a>"
                else  
       ...
       <td colspan="<%=UBound(arrFields)+3%>" style="padding-top:3px; border-top:1px solid #000;  background-color:#F8F8F8;" >
            <div style="width:100%; height:35px;">
                 ...
                 <div style="width:180px; height:25px; float:left; text-align:left; border:0px solid #F00; padding-left:25px;">
                   ...
                   <%
				   'if  VerificaDireito ("|DEL|", BuscaDireitosFromDB("mini_BreteCred",Session("PVISTA_USER_ID_USER")), false) = true then
				   %>
                    <button id="butdelete" name="butdelete" class="button link" alt="Apagar Todos Selecionados" onClick="data_DeleteSelect('msguid_','','<%=LTB%>','<%=CFG_DB%>','<%=DKN%>','<%=DLD& "?var_chavereg="&strCOD_EVENTO%>','../'); return false;">
