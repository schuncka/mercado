/* 
 ...
 WRAPPERS
	...
	MyOpenWindowWithPost(url, windowoption, name, params)
	getObj(prIDElement)
	MM_swapImgRestore()
	MM_swapImage() 
	MM_findObj(n, d)
	AutoTab(current,to)
 KERNEL (data,menu, etc...)
	data_DeleteSelect (prIdPrefix, prPagExec, prLTB, prCFGDB, prDKN, prDLD) 
	data_Paginar(prForm, prFieldCurPage, prStep, prLimit)
	data_ToggleCheckAll(prIdPrefix) 
	data_CheckAll(prIdPrefix,prValue) 
 LAYOUT
 ...
 STRING
	returnChar(prString)
	Trim(str)
	concatenaDadoCheckBox(prForm, prPrefixo, prCampoRetorno)  
 COOKIE
 	setCookie(cname, cvalue, exdays)
 	getCookie(cname)	
ESPECIFICAS
  buscaCep(prCEP, prEndereco,prBairro,prCidade,prEstado,prPais)
...
/* ----------------------------------------------------------------------------------
   ...
   function openPopupPage_ARM(prUrl, prCodInsc, prCodEvent)	{
	 ...
	 MyOpenWindowWithPost(prUrl, "width=720, height=600, left=50, top=50, resizable=yes, scrollbars=yes", "pVISTANewFile", param);
   }
   ...
   <button onClick="openPopupPage_ARM('confirmacao_arm.asp','123','3332');"></button>
  --------------------------------------------------------------------- 10/03/2017 - */
function MyOpenWindowWithPost(url, windowoption, name, params)
{
 ...
 //essa página "post.htm" serve apenas de place-holder até o conteúdo ser carregado na janela alocada
...
/* INI - Funções KERNEL ----------------------------------------------------------------------------------------- */
/* -------------------------------------------------------------------------------------------------------------- */
function data_DeleteSelect (prIdPrefix, prPagExec, prLTB, prCFGDB, prDKN, prDLD, prHPath) 
{
 var tagParam=("input" === "undefined") ? '*' : "input";    
 ...
 //--------------------------------------------------------------------------------
 // prHPath = '' normalmente vazio para modulos "pai", e '../' para modulos filhos
 //--------------------------------------------------------------------------------
 
 
 for(i=0; i < document.getElementsByTagName(tagParam).length; i++) {    
   auxID = document.getElementsByTagName(tagParam)[i].id
   if(auxID.indexOf(prIdPrefix)!=-1)  {
      if (document.getElementsByTagName(tagParam)[i].checked) {
	    if (strCods != '') {
	      strCods = strCods + ',' + document.getElementsByTagName(tagParam)[i].value;
	    } else {
	      strCods = document.getElementsByTagName(tagParam)[i].value;
	    }
	  }
   }
 } 	
 ...
 if (strCods != '') 
 {
  /* Neste caso nao poderia chamar a athDeleteToDB.asp pois precisa deletar outras coisas atreladas 
	e originalmente o Mauro chamada a deleteexec.asp local que deleta produtos e outras coisas 
	do modulo corrente (coisas precisam ser deletadas em castata pelo banco memso ou precisam ser 
	melhor analizadas.
  --------------------------------------------------------------------------------------------  */
  flag=confirm("Você quer apagar definitivamente o(s) íten(s) selecionado(s)?");
  //flag=confirm(' Por questões de Segurança este módulo esta com a opção de DELETE inativa!');
  if (flag==true) {
	  //Obs: Quando NÃO passamos um executor especial de deleção, 
	  //a rotina a ser chamada é a deleção padrão athDeletetoDB
	  if (prPagExec=='' ) {
		 //alert('../_database/athDeleteToDB.asp?default_table='+prLTB+'&default_db='+prCFGDB+'&record_key_name='+prDKN+'&record_key_value='+strCods+'&default_location='+prDLD); 
		 document.location = prHPath + '../_database/athDeleteToDB.asp?default_table='+prLTB+'&default_db='+prCFGDB+'&record_key_name='+prDKN+'&record_key_value='+strCods+'&default_location='+prDLD;
	  } else {
		 //alert(prPagExec + '?var_chavereg=' + strCods);
		 document.location = prPagExec + '?var_chavereg=' + strCods;
	  }
  }
 }
 return false;  
...
function data_Paginar(prForm, prFieldCurPage, prStep, prLimit){
 var lform = document.getElementById(prForm);
 var elem  = document.getElementById(prFieldCurPage);
 
 if (prStep=="incrementa") {
  if (elem.value < prLimit) { 
    elem.value++;
    lform.submit();
  }
 } else {
...
function data_ToggleCheckAll(prIdPrefix) {
 var tagParam = ("input" === "undefined") ? '*' : "input";    
 var elementsTable = new Array();
 var auxID = "";   
 ...
 for(var i=0 ; i < document.getElementsByTagName(tagParam).length; i++) {    
   auxID = document.getElementsByTagName(tagParam)[i].id;
   if(auxID.indexOf(prIdPrefix)!=-1)  {
      document.getElementsByTagName(tagParam)[i].checked = !document.getElementsByTagName(tagParam)[i].checked;
   }
 }  
 return false;  
}
...
function data_CheckAll(prIdPrefix,prValue) {
 var tagParam = ("input" === "undefined") ? '*' : "input";    
 var elementsTable = new Array();
 var auxID = "";   
 ...
 for(var i=0 ; i < document.getElementsByTagName(tagParam).length; i++) {    
   auxID = document.getElementsByTagName(tagParam)[i].id;
   if(auxID.indexOf(prIdPrefix)!=-1)  {
      document.getElementsByTagName(tagParam)[i].checked = prValue;
   }
 }  
 return false;  
}
...
function FormataInputDataNew(prObject,prEvt){
	...
	if(inputKey > 45 && inputKey < 58 && inputKey != 32 || inputKey == 13 || inputKey == 8 || inputKey == 0){ // NUMBERS + '/'
		if(inputKey != 127 && inputKey != 39 && inputKey != 37 && inputKey != 46 && inputKey != 45|| inputKey == 8 || inputKey == 0){
			...
			if((arrValue.length == 0 || arrValue.length == 1 || arrValue.length == 3 || arrValue.length == 4 || arrValue.length == 6 || arrValue.length == 7 || arrValue.length == 8 || arrValue.length == 9) && inputKey == 47){
				return(false);
			} else{
...
/* 
Função para concatenar dados checkbox, onde os dados retornam 
em formato de array para gravação em unico campo 
 */
function concatenaDadoCheckBox(prForm, prPrefixo, prCampoRetorno)  { 
...
/* -------------------------------------------------------------------------------------------------------------- */
/* INI - Funções ESPECIFICAS ------------------------------------------------------------------------------------ */
/* -------------------------------------------------------------------------------------------------------------- */

function buscaCep(prCEP, prEndereco,prBairro,prCidade,prEstado,prPais) {

	$(document).ready(function() {
		$.ajax({url: "../ajax/buscaCEP.asp?var_cep="+prCEP, success: function(result){																		
			console.log(result);
			arrResult = result.split("|");								
			if(arrResult[0] == 'error') {								
				return false;
			} else {													
                    $("#"+prEndereco).val(arrResult[0]);
                    $("#"+prBairro).val(arrResult[1]);
                    $("#"+prCidade).val(arrResult[2]);
                    $("#"+prEstado).val(arrResult[3]);
                    if (prPais !=""){
                        $("#"+prPais).val(arrResult[4]);
                    }
