...
Function montaForm(prTipo, prTermoTraducao, prTamanho, prNameField, prObrigatorio, prValue, prPlaceholder, prLength, prJavaScript, prFocus, prRadioValue1, prRadioValue2, pradiolabel1, pradiolabel2, pr_Combo_SQL, pr_Combo_colValue, pr_Combo_colText, pr_Combo_Codigo, pr_divId, pr_style, pr_radioCheck1, pr_radioCheck2, pr_comentario)
	...
	if prTermoTraducao <> "" Then
		...
		select case prTipo  
			...
			strField = strField &	"<select name='"&prNameField&"' id='"&tornaCampoObrigatorio2(prObrigatorio, prNameField)&"'>" & vbnewline
				...
				LimpaCampos();
					...
					<%  Do while not objRSCampos.EOF 
							if getValue(objRSCampos,"ajax_return") <> "" then 
								if getValue(objRSCampos,"tipo") = "radio" then								
					%>
									//alert("radio");
									if($("#<%=tornaCampoObrigatorio2(getValue(objRSCampos,"obrigatorio"),getValue(objRSCampos,"radio_label1"))%>").val() == arrResult[<%=getValue(objRSCampos,"ajax_return")%>])
										{alert("teste");} //$("#<%=tornaCampoObrigatorio2(getValue(objRSCampos,"radio_label1"),getValue(objRSCampos,"name_field"))%>").prop("checked", true);
										
										}
									if($("#<%=tornaCampoObrigatorio2(getValue(objRSCampos,"obrigatorio"),getValue(objRSCampos,"radio_label2"))%>").val() == arrResult[<%=getValue(objRSCampos,"ajax_return")%>])
										{alert("teste1");} //$("#<%=tornaCampoObrigatorio2(getValue(objRSCampos,"radio_label2"),getValue(objRSCampos,"name_field"))%>").prop("checked", true);}
							<%	else %>	
