...
<%
 ...
 Const DKN = "COD_PALESTRANTE_TRANSP"				 		' - Campo chave...
 Const DLD = "../pax/mini_Transporte/default.asp"  			' "../evento/data.asp" - 'Default Location após Deleção
...
' Monta SQL e abre a consulta ----------------------------------------------------------------------------------
	...
	strSQL =          " SELECT PT.COD_PALESTRANTE_EVENTO,E.NOME"
 	strSQL = strSQL & " FROM TBL_PALESTRANTE_TRANSP PT "
	strSQL = strSQL & "       INNER JOIN tbl_Palestrante_Evento PE ON PE.COD_PALESTRANTE_EVENTO = PT.COD_PALESTRANTE_EVENTO "	
            ...
            <div class="accordion place-left"  style="z-index:10; position:absolute; top:0px;">
               ...
               <a class="heading text-left bg-white fg-active-black" href="javascript:document.getElementById('formfiltro').submit();" style="height:45px;">
	                    <p class="fg-black" style="border:0px solid #FF0; padding:0px; margin:0px;">
							...
							<%= objLang.SearchIndex("menu_transporte",0)%>
                        </p>
                    </a>
