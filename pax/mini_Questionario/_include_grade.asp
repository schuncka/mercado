...
<!-- Possibilidades de tipo de sort...  class="sortable-date-dmy" / class="sortable-currency" / class="sortable-numeric" / class="sortable" //-->
    ...
    <tbody>
        ...
        <!--Menu Action INI-----------------------------------------------------------------------------------------------------//-->
             ...
             <% 'If GetValue(objRS,"SYS_DATACA") = "" Then %>
                    <div class="button-dropdown place-left" style="width:20px; height:20px; border:0px solid #F00;">
                           <img class="dropdown-toggle" src="../../img/icon_action.gif"  >                            
                             <ul class="dropdown-menu" data-role="dropdown">                                
                                <li><%=AthWindowNew ("../../modulo_Questionario/pesquisa.asp?var_chavereg="&getValue(objRS,"COD_QUESTIONARIO")&"&var_codigos="&strCodBarra&"&var_cod_insc="&strCOD_INSCRICAO&"&lng=BR", 520, 620,ucase(objLang.SearchIndex("mini_preencher",0)) )%> </li> <%'&"&lng=<%=strCOD_PAIS%>
                                <!--li><%'=AthWindowNew ("../certificadopdf.asp?var_cod_inscricao=" & GetValue(objRS,"COD_INSCRICAO") & "&var_cod_prod="&GetValue(objRS,"COD_PROD") & "&var_cod_evento="&GetValue(objRS,"COD_EVENTO")& "&var_cod_empresa=" & strCOD_EMPRESA, 520, 620, "IMPRIMIR 1")%></li//-->                                
                                <!-- li>
          ...
          <% 
            for j=0 to objRS.Fields.count-1 
              if inStr(strFields, objRS.Fields(j).name)>0 then
                ...
                if (objRS.Fields(j).name = "confirma") Then
                    if (strINFO <> "") Then
                        strINFO = Response.Write("<i class='icon-checkmark fg-green'></i>")	  
                    End If
                End If
