...
<!DOCTYPE html>
	<%		if session("METRO_ProShopPF_strByPassCortesia") = 1 AND i = 1 Then		%>
				<head>
...
function alteraValorQuantidade(prQuantidade, prCodProd, prCampoValor){
	//alert('Quantidade: ' + prQuantidade + '  / CodProd: '+ prCodProd + '  /  CampoValor: ' + prCampoValor.innerHTML);
	
	return false;
	
		$(document).ready(function(){
				$.ajax({url: "./ajax/valida_quantidade.asp?var_codigo_prod="+prCodProd+"&cod_evento=<%=strCOD_EVENTO%>&var_quantidade="+prQuantidade+"&var_categoria="+$("#var_categoria"), success: function(result){																		
							var arrResult = result.split("|")							
							if(arrResult[0] == 'err'){
								//alert(arrResult[1]);
								return false;
            			...
            			<% End If%>
                                ...
                                <% if getValue(objRS,"prc_lista") = 0 Then 'produto gratuito%>    <!-- INI: LISTA PRODUTOS //-->	
                                    <div class="tile selected" style="width:100%; height:auto; margin:0 auto;  background-color:#FFF; color:#666; text-align:LEFT; padding-top:7px; padding-left:10px; padding-bottom:25px; margin-top:15px; border:1px solid #4390DF;">
                                        <font size="+1">
                                            <%=strCategoriaTxt%> - <%=strTitulo%>(<%=strCodProd%>)
                                            <br><span style="color:#009966;" id="span_<%=i%>"><%=objLang.SearchIndex("gratuito",0)%></span>
                                        </font>
                                        <br><br><%=strDescricao%>
                                        <div class="brand">
                                            <!--span class="badge bg-lightBlue">1</span//-->
                                            <div class="badge bg-lightBlue" style="width:50px; height:auto;"> 
                                              <select name="combo_quantidade" id="combo_quantidade" onChange="alteraValorQuantidade(this.value,<%=strCodProd%>,\'span_<%=i%>\);">
                    			...
                    			<% Else if clng(getValue(objRS,"vagas")) <=0 Then 'Vagas zeradas%>
                                    ...
                                    <!--div class="tile selected"  id="createFlatWindow" style="width:100%; height:auto; margin:0 auto;  background-color:#FFF; color:#666; text-align:LEFT; padding-top:7px; padding-left:10px; padding-bottom:25px; margin-top:15px; border:1px solid #4390DF;">
                                    	...
                                    	<% Else 'Produtos a venda%>
                                                <div class="tile selected" id="tile_<%=i%>"  style="width:100%; height:auto; margin:0 auto;  background-color:#FFF; color:#666; text-align:LEFT; padding-top:7px; padding-left:10px; padding-bottom:25px; margin-top:15px; border:1px solid #4390DF;">
                                                    <font size="+1">
                                                        <%=strCategoriaTxt%> - <%=strTitulo%>(<%=strCodProd%>)
                                                        <% 'if trim(strCodProd) <> trim(strCodProdPromo) Then %>
	                                                        <br><span style="color:#0099CC;" id="span_<%=i%>">R$ <%=FormatNumber(dblValorProduto)%></span>
                                                        <%' else %>
                                                    ...
                                                    <div class="brand">
                                                        ...
                                                        <!-- Uma alternativa seria colocar COMBOS aqui como badge //-->
                                                            <div class="badge bg-lightBlue" style="width:50px; height:auto;"> 
                                                              <select name="combo_quantidade_<%=i%>" id="combo_quantidade_<%=i%>" onChange="alteraValorQuantidade(this.value,<%=strCodProd%>,span_<%=i%>);">
                                                                <option value="0" selected >0</option> 
             ...
             <div class="row" style="margin-top:15px">
                <div class="grid">
                    <div class="row">
                        ...
                        <div class="span12">
                            <a href="#" onClick="javascritp:enviaForm();">
                            <div style="width:100%; height:40px; cursor:pointer; background-color:#090; color:#FFFFFF; vertical-align:middle; text-align:center; padding-top:7px; margin-bottom:20px;">
                                <font size="+1"><b><%=objLang.SearchIndex("continuar",0)%></b></font>
                            </div>
                            </a>
                        </div>
                    </div>
