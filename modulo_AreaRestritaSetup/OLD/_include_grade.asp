                    ...
                    <div class="button-dropdown place-left" style="width:20px; height:20px; border:0px solid #F00;">
                           <img class="dropdown-toggle" src="../img/icon_action.gif"  >                            
                             ...
                             <!--DELETE comentado pois neste modo procede que não haja exclusões de registros sem a devida supervisão//-->
                                <li><%=AthWindow ("update.asp?var_chavereg=" & GetValue(objRS,DKN)&"&var_cod_evento=" & GetValue(objRS,"cod_evento") & "&var_lang=" & GetValue(objRS,"lang") , 520, 620, "ALTERAR")%></li>
                                <li><%=AthWindow ("detail.asp?var_chavereg=" & GetValue(objRS,DKN), 520, 620, "VISUALIZAR")%></li>
                                <li class="divider">
                                  <%
								  if  VerificaDireito ("|DEL|", BuscaDireitosFromDB("modulo_AreaRestritaSetup",Session("METRO_USER_ID_USER")), false) = true then
								  %>
          ...
          <% 
           for j=0 to objRS.Fields.count-1 
              if inStr(strFields, objRS.Fields(j).name)>0 then
                ...
                If (objRS.Fields(j).name = "sys_inativo") Then
                    IF (strINFO="") or ((strINFO ="null"))  Then
                        strINFO = "Não" 
                    Else
                       strINFO = "Sim" 
                    End If
