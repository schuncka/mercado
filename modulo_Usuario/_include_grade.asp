                    ...
                    <div class="button-dropdown place-left" style="width:20px; height:20px; border:0px solid #F00;">
                           <img class="dropdown-toggle" src="../img/icon_action.gif"  >                            
                             <ul class="dropdown-menu" data-role="dropdown">                                
                                ...
                                <li><%=AthWindow ("DireitosFull.asp?var_chavereg=" & GetValue(objRS,DKN), 520, 620, "DIREITOS")%></li>
                                <li><%=AthWindow ("mini_ListaEvento/default.asp?var_chavereg=" & GetValue(objRS,DKN), 720, 620, "LISTA EVENTO")%></li>
                                <li><%=AthWindow ("detail.asp?var_chavereg=" & GetValue(objRS,DKN), 520, 620, "VISUALIZAR")%></li>
                                <li class="divider">
									<%'Condição de DELETE para usuario não deletar se não tiver os direitos habilitados
								if  VerificaDireito ("|DEL|", BuscaDireitosFromDB("modulo_Usuario",Session("ID_USER")), false) then
								   %>
            ...
            for j=0 to objRS.Fields.count-1 
              if inStr(strFields, objRS.Fields(j).name)>0 then
                ...
                strINFO = Server.HTMLEncode(GetValue(objRS,objRS.Fields(j).name))
				   ...
				   'Tratamento especial exibição campo Ativo
                If (objRS.Fields(j).name = "DT_INATIVO") Then
				   ...
				   'Tratamento especial exibição campo Temporário
                If (objRS.Fields(j).name = "TEMPORARIO") Then
                    IF (strINFO="0") Then
                        strINFO = "NAO" 
                    Else
                        strINFO = "SIM"
       ...
       <td colspan="<%=UBound(arrFields)+3%>" style="padding-top:3px; border-top:1px solid #000;  background-color:#F8F8F8;" >
            <div style="width:100%; height:35px;">
                 ...
                 <div style="width:180px; height:25px; float:left; text-align:left; border:0px solid #F00; padding-left:25px;">
                    <%
				   if  VerificaDireito ("|DEL|", BuscaDireitosFromDB("modulo_Usuario",Session("METRO_USER_ID_USER")), false) then
				   %>
                 ...
                 <div style="width:150px; height:28px; float:right; text-align:center; border:1px solid #00ADEF; border-radius: 25px; background-color:#00ADEF; margin-right:5px; margin-top:5px; padding-top:0px;" >
                  <form name="formPaginar" id="formPaginar" action="default.asp" method="post">
                    ...
                    <%
					'-------------------------------------------------------------------------------------------------------------------------------------------- 
                    'Relacionado a quebra das strings em strALL_PARAMS, de maneira que os valores seja reparados e apresentado indentificando a pesquisa do filtro
