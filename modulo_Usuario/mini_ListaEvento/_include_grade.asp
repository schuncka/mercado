...
//-->                                <li><%=AthWindowNew ("detail.asp?var_chavereg=" & GetValue(objRS,"COD_USUARIO_EVENTO") & "&var_chavemaster=" & strCOD_USUARIO, 520, 620, "VISUALIZAR")%></li>
                                <li class="divider">
                                  <%
								   if  VerificaDireito ("|DEL|", BuscaDireitosFromDB("modulo_ListaEvento",Session("METRO_USER_ID_USER")), false) = true then
								   %>
								<li><a href="#" style="cursor:pointer;" onClick="Javascript:data_CheckAll('msguid_',false); document.getElementById('msguid_<%=i%>').checked=true; data_DeleteSelect('msguid_','','<%=LTB%>','<%=CFG_DB%>','<%=DKN%>','<%=DLD&"?var_chavereg="&strCOD_USUARIO%>',''); return false;">DELETAR</a></li>
									<%
									end if
									%>
                            </ul>
      ...
      <tr>
       <td colspan="<%=UBound(arrFields)+3%>" style="padding-top:3px; border-top:1px solid #000;  background-color:#F8F8F8;" >
            <div style="width:100%; height:35px;">
                   ...
                   <%
				   if  VerificaDireito ("|DEL|", BuscaDireitosFromDB("mini_ListaEvento",Session("METRO_USER_ID_USER")), false) = true then
				   %>
                    <button id="butdelete" name="butdelete" class="button link" alt="Apagar Todos Selecionados" onClick="data_DeleteSelect('msguid_','','<%=LTB%>','<%=CFG_DB%>','<%=DKN%>','<%=DLD&"?var_chavereg="&strCOD_USUARIO%>','../'); return false;">
