...
<%
 ...
 Const LTB = "fin_plano_conta"	    							' - Nome da Tabela...
 Const DKN = "COD_PLANO_CONTA"          						' - Campo chave...
 Const DLD = "../modulo_FinLctoConta/busca_planoconta.asp" 		' "../evento/data.asp" - 'Default Location após Deleção
 Const TIT = "BUSCA PLANO DE CONTA"    							' - Nome/Titulo sendo referencia como titulo do módulo no botão de filtro
...
' Monta FILTRAGEM -----------------------------------------------------------------------------------------------
 ...
 ' response.Write(strSEARCH&"<BR>")
    If isNUmeric(strSEARCH) then 
			auxSTR = auxSTR & " AND COD_PLANO_CONTA		=    		'" & Clng(strSEARCH) & "'" 
...
' Monta SQL e abre a consulta ----------------------------------------------------------------------------------
 ...
 strFields = "COD_PLANO_CONTA,COD_REDUZIDO,NOME,NIVEL,DT_INATIVO"
        ...
        <div style="width:100%;display:inline-block"> 
            ...
            <div class="grid" style="margin:0px;padding:0px;">
                        <div class="row" style="margin:0px;padding:0px;">
                              <form name="formfiltro" id="formfiltro" method="post" action="busca_planoconta.asp" >  
                                  <div class="span12">
...
<!-- Possibilidades de tipo de sort...  class="sortable-date-dmy" / class="sortable-currency" / class="sortable-numeric" / class="sortable" //-->
    ...
    <tbody>
		...
		%>
        	 onClick="Retorna('<%=GetValue(objRS,"COD_PLANO_CONTA")%>','<%=GetValue(objRS,"NOME")%>');"
        <%		
       ...
       <td colspan="<%=UBound(arrFields)+3%>" style="padding-top:3px; border-top:1px solid #000;  background-color:#F8F8F8;" >
            <div style="width:100%; height:35px;">
                 ...
                 <div style="width:150px; height:28px; float:right; text-align:center; border:1px solid #00ADEF; border-radius: 25px; background-color:#00ADEF; margin-right:5px; margin-top:5px; padding-top:0px;" >
                  <form name="formPaginar" id="formPaginar" action="busca_planoconta.asp" method="post">
